...
def delete_user(email, password) -> tuple[bool, str]:
    '''
    Function to permanently delete a user from the database.\n
    Password required for safety.
    '''
    session = SessionLocal()
...
def update_goal(email, amount) -> tuple[bool, User | str]:
    ...
    If we want to remove the goal we can use None for the amount.
    '''
    session = SessionLocal()
    try:
        user = session.query(User).filter(User.email == email).first()
...
def add_savings_transaction(user_id, amount, date):
    return create_transaction(
        user_id, -abs(amount), 2, date, "Added into savings account"
    )


def add_withdraw_transaction(user_id, amount, date):
    return create_transaction(
        user_id, abs(amount), 2, date, "Withdrawn from savings account"
    )


def add_income(user_id, amount, category_id, date, description=None):
    return create_transaction(
        user_id, abs(amount), category_id, date, description
    )
